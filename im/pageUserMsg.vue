			...
			<scroll-view class="msg-list" scroll-y="true" :scroll-with-animation="scrollAnimation"
				...
				<view class="row" v-for="(row,index) in msgList" :key="index" :id="'msg'+row.msg.id">
					...
					<block v-if="row.type=='user'">
						<!-- 自己发出的消息 -->
						<view class="my" v-if="row.msg.userinfo.uid==user.uid">
							<!-- 左-消息 -->
							<view class="left">
								<!-- 文字消息 -->
						...
						<!-- 别人发出的消息 -->
						<view class="other" v-if="row.msg.userinfo.uid!=user.uid">
							<!-- 左-头像 -->
							<view class="left">
								<image :src="row.msg.userinfo.face"></image>
							</view>
			...
			<swiper class="emoji-swiper" :class="{hidden:hideEmoji}" indicator-dots="true" duration="150">
				<swiper-item v-for="(page,pid) in emojiList" :key="pid">
					<view v-for="(em,eid) in page" :key="eid" @tap="addEmoji(em)">
						<image mode="widthFix" :src="'/im/static/img/emoji/'+em.url"></image>
					</view>
				</swiper-item>
			</swiper>
			<!-- 更多功能 相册-拍照-红包 -->
			...
			<view class="layer" @touchmove.stop.prevent="discard">
				<view class="open-redenvelope">
					<view class="top">
						...
						<image src="@/im/static/img/im/face/face_1.jpg"></image>
					</view>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			console.log(option)
			// this.getUserMsg(option.uid);
			uni.setNavigationBarTitle({
				title: option.title
			})
			this.uid = option.uid;
			this.getMsgList(option.uid);
			//语音自然播放结束
		...
		onShow() {
			...
			// 定时接收消息处理
			this.timer = setInterval(this.getMsgList, 2000)
		...
		methods: {
			...
			screenMsg(msg) {
				...
				if (msg.type == 'system') {
					...
					console.log('用户消息');
					//非自己的消息震动
					if (msg.msg.userinfo.uid != this.user.uid) {
						console.log('振动');
						uni.vibrateLong();
					}
				}
			...
			loadHistory(e) {
				...
				const self = this;
				var params = {
					url: 'getUserMsg?uid=' + this.uid + '&id=' + Viewid,
					sCallBack: function(res) {
						console.log(res);
						let list = res.list;

						if (self.msgList[0].msg.content.text !== '暂无聊天记录') {
							// 获取消息中的图片,并处理显示尺寸
							for (let i = 0; i < list.length; i++) {
								if (list[i].type == 'user' && list[i].msg.type == "img") {
									list[i].msg.content = self.setPicSize(list[i].msg.content);
									self.msgImgList.unshift(list[i].msg.content.url);
								}
								// list[i].msg.id = Math.floor(Math.random()*1000+1);
								// list[i].msg.id = list[i].msg.id;
								self.msgList.unshift(list[i]);
							}
						}
				...
				var params = {
					...
					sCallBack: function(res) {
						...
						for (let i = 0; i < list.length; i++) {
							if (list[i].type == 'user' && list[i].msg.type == "img") {
								list[i].msg.content = self.setPicSize(list[i].msg.content);
								self.msgImgList.push(list[i].msg.content.url);
							}
						}
						self.msgList = list;
						// 滚动到底部
						...
						self.$nextTick(function() {
							// 滚动到底
							self.scrollToView = 'msg' + list[list.length - 1].msg.id
						});
			...
			sendMsg(content, type) {
				...
				var params = {
					...
					sCallBack: function(res) {
						...
						let msg = res.list[0];
						// 发送消息
						let lastid = self.msgList[self.msgList.length - 1].msg.id;
						lastid++;
						msg.msg.id = lastid;
						self.screenMsg(msg);
					}
			...
			toDetails(rid) {
				uni.navigateTo({
					url: '@/im/HM-details/HM-details?rid=' + rid
				})
			},
			// 预览图片
			showPic(msg) {
...
<style lang="scss">
	@import "@/im/static/HM-chat/css/style.scss";
</style>
