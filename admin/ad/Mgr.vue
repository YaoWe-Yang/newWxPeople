		...
		<div class="list">
			...
			<list v-model="list.loading" :finished="list.finished" @load="$getList('adAdd', list)">
				<div class="item" v-for="(item, key) in list.data" :key="key">
					...
					<!-- 隐藏按钮 -->
					<div class="dispBut">
						<!-- <u-switch :v-model="[item.is_hidden ? true:false]" :active-value="item.id" :inactive-value="item.id"></u-switch> -->
						<switch @change="switchChange(item.id)" style="transform:scale(0.8)" v-if="item.is_hidden" />
						<switch checked @change="switchChange(item.id)" style="transform:scale(0.8)" v-else />
					</div>
					...
					<div class="bottom">
						<div class="left">
							<template v-if="item.status==1 && item.is_hidden==0"><text
									style="color: #ff9900;">审核中</text></template>
							<template v-if="item.status==2 && item.is_hidden==0"><text
									style="color: #19be6b;">代发中</text></template>
							<template v-if="item.status==3 && item.is_hidden==0"><text
									style="color: #fa3534;">已结束</text></template>
				...
				<div class="users">
					<div class="item" v-for="(item, key) in note_dialog.list.data" :key="key">
						<div class="user">
							...
							<div class="info">
								<div class="nickname">{{item.user.nickname?item.user.nickname:'游客'}}</div>
