		...
		<div class="list">
			<list v-model="list.loading" :finished="list.finished" @load="$getList('getMyQunList', list)">
				<div v-for="(item, key) in list.data" class="item" :key="key">
					<div class="avatar">
						<u-lazy-load
							:image="'https://wqrm.oss-cn-shenzhen.aliyuncs.com/'+item.avatar+'?x-oss-process=image/resize,m_fill,h_92,w_92'"
							mode="widthFix"></u-lazy-load>
					</div>
					<div class="profile">
						<div class="nickname">
						  <text class="userCardNickname">{{item.nickname}}</text>
						  <i class="sexIcon man" v-if="item.sex=='男'">&#xe7b2;</i>
						  <i class="sexIcon woman" v-else-if="item.sex=='女'">&#xe7b1;</i>
						</div>
						<div class="region" v-if="item.region!=''">{{item.region}}</div>
						<div class="summary" v-if="item.summary!='' && item.summary!==null">{{item.summary}}</div>
				...
				<view class="dfAd" @click="$go('Add')" v-if="list.data.length!==0">发布名片</view>
			</list>
...
<script>
	...
	export default {
		...
		computed: {
			...
			chargeList() {
				let list = [];
				var top_charge = this.config.card.top_charge;
						...
						if (res.confirm) {
							...
							var params = {
								...
								sCallBack: function(res) {
									uni.hideLoading();
									if (res.code == 0) {
										myList.splice(myList.indexOf(card), 1);
										uni.showToast({
											title: '删除成功',
											duration: 2000
										});
